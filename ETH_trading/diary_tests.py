...
def add_trade(diary, trade):
    ...
    with pd.ExcelWriter(path=diary, engine='openpyxl', datetime_format='DD-MM-YYYY hh:mm', mode='a') as \
        ...
        writer.book['Sheet1'].insert_rows(2)
        trade.to_excel(writer, sheet_name='Sheet1', startrow=1, header=None, index_label='date')
        writer.close()


def get_latest_trade(diary):
    return pd.read_excel(diary, nrows=1)
